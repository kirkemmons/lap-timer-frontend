      ...
      v-container.fluid.fill-height
        ...
        v-row(justify="center")
          v-col(cols="9" lg="6")
            v-data-table.mt-4(
              :headers="headers"
              :items="laps.filter(lap => lap.carId === carId)"
              class="elevation-18"
...
export default {
  mixins: [makeFindMixin({ service: 'laps', watch: true }), makeGetMixin({ service: 'cars', watch: true })],
  ...
  methods: {
    ...
    async createLap () {
      ...
      const data = { time: lapTimeDuration, carId }
